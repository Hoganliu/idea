...
int alg_des(unsigned char *dst, unsigned char *src, unsigned char *key, int mode)
{
    ...
    if (mode == ENCRYPT_MODE) {
        ...
        ByteToBit(key, 8, bits);

        Des_GenSubKey(bits, subKey);

        ByteToBit(src, 8, bits);
        Des_IP(bits, strTmp);
        memcpy(L0, strTmp, 32);
...
int alg_3des(unsigned char *dst, unsigned char *src, unsigned char *key, int mode)
{
    ...
    if (mode == ENCRYPT_MODE) {
        alg_des(tmp, src, key, ENCRYPT_MODE);
        alg_des(tmp, tmp, key + 8, DECRYPT_MODE);
        alg_des(dst, tmp, key + 16, ENCRYPT_MODE);
    } else {
        alg_des(tmp, src, key + 16, DECRYPT_MODE);
        alg_des(tmp, tmp, key + 8, ENCRYPT_MODE);
        alg_des(dst, tmp, key, DECRYPT_MODE);
    }
...
{
    int i, j;
    unsigned char tmp[8], dst_tmp[len];
    ...
    if (mode == ENCRYPT_MODE) {
        for (i = 0; i < len; i += 8) {
            for (j = 0; j < 8; j++)
                tmp[j] ^= src[i + j];
            alg_3des(dst_tmp + i, tmp, key, ENCRYPT_MODE);
            memcpy(tmp, dst_tmp + i, 8);
        }
    } else {
        for (i = 0; i < len; i += 8) {
            alg_3des(dst_tmp + i, src + i, key, DECRYPT_MODE);
            for (j = 0; j < 8; j++)
                dst_tmp[i + j] ^= tmp[j];
            memcpy(tmp, src + i, 8);
        }
...
/* des algorithm PC-1 transformation, transform 64-bits key K to 56-bits */
...
/* des algorithm PC-2 transformation, transform 56-bits key K to 48-bits */
